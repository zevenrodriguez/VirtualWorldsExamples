...
function MudText(options) {
    ...
    // Try multiple ways to create a canvas across different JS environments
    let canvas = null;
    if (typeof window !== 'undefined' && window.document) {
        canvas = window.document.createElement('canvas');
    } else if (renderer.domElement && renderer.domElement.ownerDocument) {
        canvas = renderer.domElement.ownerDocument.createElement('canvas');
    }

    if (!canvas) {
        console.error('MudGUI: could not create canvas element');
        return null;
    }

    canvas.width  = canvasW;
    canvas.height = canvasH;
    console.log('MudGUI canvas created:', canvasW, 'x', canvasH);
